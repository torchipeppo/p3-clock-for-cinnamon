...
def date_from_isoformat_or_today(iso):
    try:
        return datetime.date.fromisoformat(iso)
    except (TypeError, ValueError):  # first catches None, second weird cases like "null"
...
class ListOfDates(SettingsWidget):
    ...
    def __init__(self, info, key, settings):
        ...
        dep_key=info.get("dependency", None)
        tooltip=info.get("tooltip", "")

        self.storage = info["storage"]
        # TODO storage come nel kdeapplet
        ...
        for i in range(len(columns)):
            ...
            if column_def['type'] == "date":     # TODO
                def map_func(col, rend, model, row_iter, data):
                    date_str = date_from_isoformat_or_today(model[row_iter][data]).strftime("%x")
        ...
        for i in range(len(self.columns)):
            ...
            if info is not None and info[i] is not None:
                ...
                if isinstance(widget, DateChooser):
                    the_info = date_from_isoformat_or_today(the_info)
                    the_info = {"y": the_info.year, "m": the_info.month, "d": the_info.day}
            ...
            for widget in widgets:
                ...
                if isinstance(widget, DateChooser):
                    ...
                    if value is None:
                        ...
                        date = datetime.date(value["y"], value["m"], value["d"])
